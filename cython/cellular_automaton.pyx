...
cdef class PythonClassName: 
  ...
  def __cinit__(self, jump_param_composites = "default" , subaggregate_threshold = "default"):
    ...
    if subaggregate_threshold ==  "default": subaggregate_threshold= 0.1
  ...
  def place_particle(self, _jump_parameter, _shape, _nx_base, _position, _face_charge, _properties): 
    if _jump_parameter == "default" : _jump_parameter = 1.0 
    if _shape == "default" : _shape = [ 0,1 ]
    if _nx_base == "default" : _nx_base = [100,100]
    if _position == "default": _position = -1 
    if _face_charge == "default": _face_charge = [ 0 ]* 2 
    if _properties == "default": _properties = [1]
    return self.thisptr.place_particle(_jump_parameter, _shape,_nx_base, _position, _face_charge, _properties)
  ...
  def porosity_d(self):
    return self.thisptr.porosity_d()
